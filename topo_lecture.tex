...
\begin{example}
    ...
    and  for \(G = (\IF_2, +)\), \(A = \IF_2\) we have \(0 \neq x \coloneq [\Id_{\IF_2}] \in H^1(B\IF_2, \IF_2)\).

    We will show that \(x^n = x \cup \dots \cup x\) (\(n\)-times) \(\in H^n(B\IF_2, \IF_2)\) is nonzero.
    \begin{Proposition}
        \(x^n \in H^n(B\IF_2, \IF_2)\) is represented by \(f_n \colon (\IF_2)^n \to \IF_2, f_n(\lambda_1, \dots, \lambda_n) = \lambda_1 \cdot \dots \cdot \lambda_n = \begin{cases}
    ...
    Claim: \(x^n \neq 0\). In the UCT for cohomology we uset the evaluation pair
    \[\Psi\colon H^n(X,A) \to \Hom(H_n(X;\IZ);A), \quad [f_n\colon X_n \to A] \mapsto \set{[\sum b_ix_i] \mapsto \sum b_i f(x_i)}\]
    for \(b_i \in \IZ, x_i \in X_n\).
    We can sligtly variate that for ring coefficients:
    \[\Psi\colon H^n(X,R) \to \Hom(H_n(X,R), R)\]
    ...
    So
    \[\Psi(x^n) \colon H_n(B\IF_2, \IF_2) \to \IF_2\]
    \[\Psi(x^n)[y] = \Psi[f_n][\sum_{(\lambda_1, \dots, \lambda_n) \in \IF_2^n}(\lambda_1, \dots, \lambda_n)] = \sum_{(\lambda_1, \dots, \lambda_n)} f_n(\lambda_1, \dots, \lambda_n) = \sum_{(\lambda_1, \dots, \lambda_n)} \lambda_1,\cdot \dots \cdot \lambda_n = 1 \neq 0\]
    and \([y] \neq 0\) in \(H_n(B\IF_2, \IF_2)\).


    An outlook: \(H^*(B\IF_2;\IF_2) = \IF_2[X]\).
...
\begin{thm}{Künneth, simple version}{}
    Let \(X\) and \(Y\) be spac3es and \(k\) a field. Then
    \[H_n(X\times Y, k)\]
    is natural isomorphic to
    \[\bigoplus_{p+q = n} H_p(X,k) \otimes_k H_q(Y,k)\]
\end{thm}

\subsection{The Eilenberg Zilber Map theorem}
...
\begin{defi}{Simplicial abelian group}{}
    ...
    Equivalently a colleciton of abelian groups \(A_n\), and homomorphisma \(\alpha^*\colon A_m \to A_n\) for all \(\alpha\colon [n] \to [m]\) in \(\Delta\), s.t. \((\alpha\circ \beta)^* = \beta^* \circ \alpha^*\).

    Equivalently a simplical set endorsed with abelian group stuctur on the sets of \(n\)-simplices, such that all \(\alpha^*\) are homomorphisms.
\end{defi}
...
\begin{remark}
    The tensor product of chain complexes \(c_* , d*\) of chain complexes is
    ...
    For \(A,B \in (\mathbf{SAB}) = \) simplicial abelian groups
    \[C_*(A\otimes B) \neq C_*(A) \otimes C_*(B)\]
    Also he did this in dimension \(n\), but I lacked time to copy.

    The Eilenberg-Ziller theorem is a natural pair of chain homotopy equivalences between these two.
\end{remark}

\begin{construction}
    Let \(A,B\) be simplicial chain groups. The \emph{Alexander-Whitney map} is the cain map
    \[AW\colon C_*(A\otimes B) \to C_*(A) \otimes C_*(B)\]
    defined by
    \[\begin{tikzcd}
        C_n(A\otimes B) \ar[d, phantom, sloped, "="]\ar[r] & \bigoplus_{p+q = n, p,q \geq 0} A_p \otimes B_q \ar[d, phantom, sloped, "="] \\
