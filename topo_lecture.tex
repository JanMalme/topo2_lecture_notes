...
\begin{example}[Graßmann manifolds]
...
\begin{defi}{Cap-Product}{}
    ...
    \[\cap \colon C_n(X, Y ; R) \times C^{i}(X,Y, R) \to C_{n-1}(X;R)\]
    given by
...
\begin{proposition}
    \(Y \subseteq X\) simplicial sets, \(R\) some commutative ring. Then the following hold:
    \begin{enumerate}
...
To solve this, we introduce Compactly supported cohomology as a variation of singular cohomology that has Poincare duality for not necessarily compact manifolds: We will get
...
For compact oriented manifolds \(M\), Poincáre duality says that
...
We now prooved Poincáre duality for \(\IR^n\). This is in itself of course not very exciting, but the first step for the bootstrapping we will do later. For that we will develop a Mayer-Vietoris sequence for compactly supported cohomology.

We now first need

\subsection{Naturality of the duality map}
